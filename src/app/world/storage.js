...
(function(global) {
	...
	class StorageManager {
		constructor(dbName = 'metaos_core_db', storeName = 'snapshots') {
			this.dbName = dbName; this.storeName = storeName; this.db = null; this.initPromise = this._initDB();
        ...
        async saveSystemState(vfsFiles, stateSnapshot) {
            await this.ready();
            const payload = { files: vfsFiles, state: stateSnapshot, timestamp: Date.now() };
            return new Promise((resolve, reject) => {
		...
		async createSnapshot(label, vfsFiles, stateSnapshot) {
			...
			const snapshot = { id, label, timestamp: Date.now(), files: vfsFiles, state: stateSnapshot };
			return new Promise((resolve, reject) => {
