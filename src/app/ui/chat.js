...
(function(global) {
	...
	class ChatComponent {
		...
		renderUploadPreview(file) { if (!this.els.filePreviewArea) return; this.els.filePreviewArea.classList.remove('hidden'); const div = document.createElement('div'); div.className = "bg-gray-800 border border-gray-600 rounded px-2 py-1 text-xs flex items-center gap-2 text-gray-300 animate-fade-in"; div.innerHTML = `<span>📎 ${file.name}</span>`; this.els.filePreviewArea.appendChild(div); }
		...
		_createTagHTML(match) {
			...
			if (tagName === 'thinking') { title = "💭 Thinking"; colorClass = "border-blue-900 bg-blue-900/20"; }
			else if (tagName === 'plan') { title = "📅 Plan"; colorClass = "border-green-900 bg-green-900/20"; }
			else if (tagName === 'event') { title = "⚡ System Event"; colorClass = "border-purple-900 bg-purple-900/20"; }
			else if (tagName === 'report' || tagName === 'ask') { title = tagName === 'ask' ? "❓ Question" : "📢 Report"; colorClass = "border-indigo-900 bg-indigo-900/40"; isOpen = true; }
			else if (tagName === 'finish') { title = "✅ Completed"; colorClass = "border-green-600 bg-green-900/60"; isOpen = true; }
			else if (['create_file', 'edit_file'].includes(tagName)) { const pathMatch = attributes.match(/path=["']?([^"'\s]+)["']?/); title = `📝 ${tagName}: ${pathMatch ? pathMatch[1] : ''}`; colorClass = "border-yellow-900 bg-yellow-900/20"; }
			else if (tagName === 'get_time') { title = "🕒 Get Time"; colorClass = "border-cyan-900 bg-cyan-900/20"; } // Specific style for get_time
            else { title = `🔧 ${tagName}`; } // Fallback title style
