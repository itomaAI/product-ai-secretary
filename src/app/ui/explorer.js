	...
	class ExplorerComponent {
		...
		_bindTreeEvents() {
			this.treeView.on('open', (path) => {
				if (this.events['open_file']) this.events['open_file'](path, this.vfs.readFile(path));
			});
			...
			this.treeView.on('create_folder', (path) => {
				this._emitHistoryEvent('folder_created', this.vfs.createDirectory(path));
			});
			...
			this.treeView.on('rename', (oldPath, newPath) => {
				this._emitHistoryEvent('file_moved', `User action: ${this.vfs.rename(oldPath, newPath)}`);
			});
			this.treeView.on('move', (srcPath, destPath) => {
				try {
					this._emitHistoryEvent('file_moved', `User moved file (drag&drop): ${this.vfs.rename(srcPath, destPath)}`);
				} catch (e) {
					alert(`Move failed: ${e.message}`);
				}
			});
			this.treeView.on('delete', (path) => {
				this._emitHistoryEvent('file_deleted', `User action: ${this.vfs.deleteFile(path)}`);
			});
			this.treeView.on('download', (path) => {
				try {
					this._downloadFile(path, this.vfs.readFile(path));
				} catch (e) {
			...
			if (this.sidebar) {
				...
				this.sidebar.addEventListener('drop', async (e) => {
					...
					for (let i = 0; i < items.length; i++) {
						...
						if (item) promises.push(this._traverseFileTree(item, ""));
					}
					const fileEntries = (await Promise.all(promises)).flat();
					if (fileEntries.length > 0) this._batchWriteFiles(fileEntries);
				});
			}
		}
		...
		_emitHistoryEvent(type, description) {
			if (this.events['history_event']) this.events['history_event'](type, description);
		}
		...
		_initResizer() {
			...
			const move = (e) => {
				...
				if (newWidth > 150 && newWidth < 600) this.sidebar.style.width = `${newWidth}px`;
			};
