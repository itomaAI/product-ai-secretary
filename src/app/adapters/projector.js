...
(function(global) {
	...
	class MetaForgeProjector extends global.REAL.ContextProjector {
		constructor(systemPrompt) { super(); this.systemPrompt = systemPrompt; }
		createContext(state) {
            ...
            } catch (e) { console.warn("Failed to load persona config", e); }

            const now = new Date();
            const timePrompt = `\n\n<system_info>\nCurrent Time: ${now.toLocaleString()}\nTimestamp: ${now.toISOString()}\n</system_info>`;
            
			apiMessages.push({ role: 'user', parts: [{ text: this.systemPrompt + configPrompt + timePrompt }] });
		...
		_convertTurnToParts(turn) {
			...
			if (Array.isArray(turn.content)) {
				if (turn.meta && turn.meta.type === TurnType.TOOL_EXECUTION) {
					...
					turn.content.forEach(c => {
						if (c.output && c.output.image) {
							parts.push({ inlineData: { mimeType: c.output.mimeType || 'image/png', data: c.output.image } });
						}
					});
					return parts;
				}
				if (turn.role === Role.USER) {
					const parts = [];
					let textBuffer = "";
					const flushText = () => { if (textBuffer.trim()) { parts.push({ text: `<user_input>\n${textBuffer.trim()}\n</user_input>` }); } textBuffer = ""; };
					for (const item of turn.content) {
						if (item.text) textBuffer += item.text + "\n";
						else if (item.inlineData) { flushText(); parts.push({ inlineData: item.inlineData }); }
					}
					flushText();
					return parts;
				}
				...
				if (turn.meta && turn.meta.type === TurnType.TOOL_EXECUTION) {
					turn.content.forEach(item => {
						if (item.output && item.output.image) {
							if (foundLatestImage) { delete item.output.image; item.output.log += "\n[System: Old screenshot removed]"; }
							else { foundLatestImage = true; }
						}
					});
				}
			}
